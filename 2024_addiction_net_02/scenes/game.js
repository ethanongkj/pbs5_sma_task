var SceneGame = new Phaser.Class({
    ...
    create: function() {
        ...
        // if they accept the offer (and in all practice trials)
            ...
            this.netflixSpeed = (window.innerWidth*this.placeGoal-window.innerWidth*0.1)/this.timer
            this.time.addEvent({
                delay: this.timer*1000,
                callback: () => {
            ...
            // netflix symbol
            this.netflix = this.physics.add.sprite(window.innerWidth*0.1, window.innerHeight*0.375, 'tiktok');
            this.netflix.setScale(1 * scaleFactor);
            this.netflix.setOrigin(0, 0.5);
            this.anims.create({
                key: 'netflixswim',
                frames: this.anims.generateFrameNumbers('netflix', { start: 0, end: 1 }),
                frameRate: 7,
                repeat: -1,
                yoyo: true
            });
            this.netflix.anims.play('netflixswim', true);
            this.netflix.setVelocityX(this.netflixSpeed);
            this.netflix.setCollideWorldBounds(true);
    ...
    updateTimeBar: function(intitialTimeBar) {
        ...
        if(globalThis.data.choice[globalThis.data.choice.length-1] == 0){
            ...
            if(Math.floor(this.netflix.x - this.squid.x) > 0){
                this.winText.setText(``);
            } else {
