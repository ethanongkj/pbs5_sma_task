var SceneGame = new Phaser.Class({
    ...
    create: function() {
        ...
        // if they accept the offer (and in all practice trials)
            ...
            this.tiktokSpeed = (window.innerWidth*this.placeGoal-window.innerWidth*0.1)/this.timer
            this.time.addEvent({
                delay: this.timer*1000,
                callback: () => {
            ...
            // tiktok symbol
            this.tiktok = this.physics.add.sprite(window.innerWidth*0.1, window.innerHeight*0.375, 'tiktok');
            this.tiktok.setScale(1 * scaleFactor);
            this.tiktok.setOrigin(0, 0.5);
            this.anims.create({
                key: 'tiktokswim',
                frames: this.anims.generateFrameNumbers('tiktok', { start: 0, end: 1 }),
                frameRate: 7,
                repeat: -1,
                yoyo: true
            });
            this.tiktok.anims.play('tiktokswim', true);
            this.tiktok.setVelocityX(this.tiktokSpeed);
            this.tiktok.setCollideWorldBounds(true);
    ...
    updateTimeBar: function(intitialTimeBar) {
        ...
        if(globalThis.data.choice[globalThis.data.choice.length-1] == 0){
            ...
            if(Math.floor(this.tiktok.x - this.squid.x) > 0){
                this.winText.setText(``);
            } else {
