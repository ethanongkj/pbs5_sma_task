var SceneCountdown = new Phaser.Class({
  ...
  create: function () {
    ...
    seabed = this.add
      .image(window.innerWidth * 0.5, window.innerHeight, "seabed")
      .setOrigin(0.5, 1);
    ...
    if (!(globalThis.data.choice[globalThis.data.choice.length - 1] == 0)) {
      this.add
        .text(
          ...
          `Start clicking in`,
          {
            fontSize: 50 * globalThis.scaleFactor,
            color: "#000000",
            lineSpacing: 5,
            align: "center",
          }
    ...
    this.countText = this.add
      .text(window.innerWidth * 0.5, window.innerHeight * 0.5, initialTime, {
        fontSize: 140 * globalThis.scaleFactor,
        color: "#000000",
        lineSpacing: 5,
        align: "center",
      })
    ...
    if (
      globalThis.data.phase[globalThis.data.phase.length - 1] == "calibration"
    ) {
      this.time.addEvent({
        delay: 2000,
        callback: () => {
